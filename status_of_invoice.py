...
old_mis = pd.read_excel(r"C:\Users\hrithik.chauhan\Downloads\data (17).xlsx")
...
open_status_ssc = mis_asn[mis_asn['Status of Invoice'].isin(list_open_status_ssc)]
...
#
#
master = pd.read_excel(r"C:\Users\hrithik.chauhan\Downloads\data (17).xlsx")
...
for i in range(0, len(master)):
    ...
    if master['Docket No.'][i] in master_car_wf:
        # if car['WF Status'] == "WFAPPROVED":
            ...
            check_list1 = []
            check_list1.append(master['Docket No.'][i])
            print(check_list1)
            mid_car = car[car['Document ID'].isin(check_list1)]
            if mid_car['WF Status'].iloc[0] == "WFAPPROVED":
                master.loc[i, 'Status of Invoice'] =  'Pending with SSC'
                master.loc[i, 'Reason of Rejection/Hold'] =  'workflow approval'
            mid_car_or = car[car['Document ID'].isin(check_list1)]
